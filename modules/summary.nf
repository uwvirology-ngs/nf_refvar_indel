process SUMMARY {
    ...
    def gene_length = params.genomic_region_len
    ...
    # cds coverage
    pct_cds_covered=\$(samtools coverage -r ${params.genomic_region} ${map_ref_bam} | awk 'NR>1' | cut -f6)
    pct_cds_covered_formatted=\$(printf "%.2f" "\${pct_cds_covered}")
    mean_cds_coverage=\$(samtools coverage -r ${params.genomic_region} ${map_ref_bam} | awk 'NR>1' | cut -f7)
    mean_cds_coverage_formatted=\$(printf "%.2f" "\${mean_cds_coverage}")
    num_bases_cds_50x=\$(samtools depth -r ${params.genomic_region} ${map_ref_bam} | awk '{if(\$3>50)print\$3}' | wc -l)
    pct_cds_50x=\$(echo "\${num_bases_cds_50x}/${gene_length}*100" | bc -l)
    pct_cds_50x_formatted=\$(printf "%.2f" "\${pct_cds_50x}")
    num_bases_cds_100x=\$(samtools depth -r ${params.genomic_region} ${map_ref_bam} | awk '{if(\$3>100)print\$3}' | wc -l)
    pct_cds_100x=\$(echo "\${num_bases_cds_100x}/${gene_length}*100" | bc -l)
    ...
    echo "sample_name\traw_reads\ttrimmed_reads\tpct_reads_trimmed\tmapped_reads\tpct_reads_mapped\tpct_genome_covered\tmean_genome_coverage\tpct_cds_covered\tmean_cds_coverage\tpct_cds_50x\tpct_cds_100x\tconsensus_length\tnum_ns\tpct_ns\tnum_ambiguous" > ${prefix}_summary.tsv
    echo "${prefix}\t\$raw_reads\t\$trimmed_reads\t\${pct_reads_trimmed_formatted}\t\${mapped_reads}\t\${pct_reads_mapped_formatted}\t\${pct_genome_covered_formatted}\t\${mean_genome_coverage_formatted}\t\${pct_cds_covered_formatted}\t\${mean_cds_coverage_formatted}\t\${pct_cds_50x_formatted}\t\${pct_cds_100x_formatted}\t\${consensus_length}\t\${num_ns_consensus}\t\${pct_ns_formatted}\t\${num_non_ns_ambiguous}" >> ${prefix}_summary.tsv
    """
}
